...
echo "=== javac (모든 src 파일) ==="
javac -source 8 -target 8 -bootclasspath "$(cygpath -w "$AJ")" -d build/classes \
  gen/com/hongcha/tesla/R.java \
  src/com/hongcha/tesla/*.java 2>&1 | grep -viE "warning|obsolete|deprecat|^Note" || true

echo "=== d8 ==="
cmd //c "$(cygpath -w "$BT/d8.bat")" --min-api 26 --output build \
  --lib "$(cygpath -w "$AJ")" build/classes/com/hongcha/tesla/*.class
